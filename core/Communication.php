<?php
    ...
    class AccountComment implements Communication {
        protected $connection;
        protected $Main, $Lib, $Database;
        ...
        public function getData(int $accountID = 0, int $userID = 0, int $page, int $getSent = 0, int $levelID = 0, int $gameVersion = 0, int $binaryVersion = 0): string {
            ...
            $comment = $this->connection->prepare("SELECT comment, userID, likes, isSpam, commentID, timestamp FROM acccomments WHERE userID = :userID ORDER BY timeStamp DESC LIMIT 10 OFFSET $page");
            $comment->execute([":userID" => $userID]);
            $commentResult = $comment->fetchAll();

            if ($commentResult == 0) return "#0:0:0";

            $commentCount = $this->connection->prepare("SELECT count(*) FROM acccomments WHERE userID = :userID");
            $commentCount->execute([":userID" => $userID]);
            $commentCountResult = $commentCount->fetchColumn();

            foreach ($commentResult as $comments) {
                if ($comments["commentID"] != 0)
                {
                    $this->uploadDate = $this->Lib->make_time($comments["timestamp"]);
                    $commentString .= "2~".$comments["comment"]."~3~".$comments["userID"]."~4~".$comments["likes"]."~5~0~7~".$comments["isSpam"]."~9~".$this->uploadDate."~6~".$comments["commentID"]."|";
                }
            }
            
            $commentString = substr($commentString, 0, -1);

            return $commentString."#".$commentCountResult.":".$page.":10";
        }
        ...
        public function upload(int $accountID, int $userID, int $levelID = 0, int $toAccountID = 0, string $userName = "", $comment = "", string $subject = "", string $body = "", $secret = ""): string {
            $commentAcc = $this->connection->prepare("INSERT INTO acccomments (userName, comment, userID, timeStamp) VALUES (:userName, :comment, :userID, :uploadDate)");
	        $commentAcc->execute([':userName' => $userName, ':comment' => $comment, ':userID' => $userID, ':uploadDate' => $this->uploadDate]);
