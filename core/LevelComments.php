<?php
    ...
    class LevelComments implements LevelCommentsInterface {
        ...
        public function get_data(int $user_id, int $level_id, int $page): string {
            ...
            $comment_count = $this->db->count(
                "comments $this->user_list_join",
                "$this->filter_to_filter$this->filter_column",
                [ ":filterID" => $this->filter_id ]
            );
            
            if ($comment_count == 0) return "-2";

            $comments = $this->db->fetch_all(
                "SELECT comments.levelID, comments.commentID, comments.timestamp, comments.comment, comments.userID, comments.likes, comments.isSpam, comments.percent, users.userName, users.icon, users.color1, users.color2, users.iconType, users.special, users.extID FROM comments LEFT JOIN users ON comments.userID = users.userID $this->user_list_join WHERE comments.$this->filter_column = :filterID $this->user_list_where ORDER BY comments.$this->mode_column DESC LIMIT $this->count OFFSET $page",
                [ ":filterID" => $this->filter_id ]
            );
            ...
            if ($this->binary_version < 32) {
                ...
                return $comment_string . "#" . $user_string . "#" . $comment_count . ":" . $page . ":" . $comment_count;
            }

            return $comment_string . "#" . $comment_count . ":" . $page . ":" . $comment_count;
        }

        public function delete(int $account_id, int $user_id, int $comment_id, int $permission): string {
            $comment = $this->db->execute(
                "DELETE FROM comments WHERE commentID = :commentID AND userID = :userID LIMIT 1",
                [
                    ":userID" => $user_id,
                    ":commentID"=> $comment_id,
                ]
            );
            $comment = $this->db->row_count();

            if ($comment == 0) return "-1";

            $comment = $this->db->fetch_column(
                "SELECT users.extID FROM comments INNER JOIN levels ON levels.levelID = comments.levelID INNER JOIN users ON levels.userID = users.userID WHERE commentID = :commentID",
                [ ":commentID" => $comment_id ] 
            );

            if ($comment == $account_id || $permission) {
                $comment = $this->db->execute(
                    "DELETE FROM comments WHERE commentID = :commentID LIMIT 1",
                    [ ":commentID" => $comment_id]
                );
            }
        ...
        public function upload(int $account_id, int $user_id, string $user_name, string $comment, int $level_id): string {
            ...
            $this->db->insert(
                "INSERT INTO comments (userName, comment, levelID, userID, timeStamp, percent) VALUES (:userName, :comment, :levelID, :userID, :uploadDate, :percent)",
                [
                    ":userName" => $user_name, 
                    ":comment" => $comment, 
                    ":levelID" => $level_id, 
            ...
            if ($this->percent != 0) {
                $comment_result = $this->db->fetch_column(
                    "SELECT percent FROM levelscores WHERE accountID = :accountID AND levelID = :levelID",
                    [":accountID" => $account_id, ":levelID" => $level_id]
                );

                if ($comment_result < $this->percent) {
                    $this->db->execute(
                        "UPDATE levelscores SET percent = :percent, uploadDate = :uploadDate WHERE accountID = :accountID AND levelID = :levelID",
                        [":percent" => $this->percent, ":uploadDate" => $this->upload_date, ":accountID" => $account_id, ":levelID" => $level_id]
                    );
                }
            } 
